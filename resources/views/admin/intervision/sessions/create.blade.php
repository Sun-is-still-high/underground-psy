...
@section('title', 'Планирование сессии — ' . $group->name)
...
<div class="container">
    ...
    <p class="subtitle">Группа: <strong>{{ $group->name }}</strong></p>

    <div class="form-container">
        <form action="{{ route('admin.intervision.sessions.store', $group->id) }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="text" id="topic" name="topic" required
                       ...
                       class="{{ $errors->has('topic') ? 'is-invalid' : '' }}">
                @error('topic') <span class="error-text">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="description">Описание</label>
                <textarea id="description" name="description" rows="3"
                          placeholder="Дополнительная информация о сессии">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    @error('scheduled_at') <span class="error-text">{{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label for="duration_minutes">Длительность (мин)</label>
                    <input type="number" id="duration_minutes" name="duration_minutes"
                           min="30" max="240" value="{{ old('duration_minutes', 90) }}">
                </div>
            </div>
